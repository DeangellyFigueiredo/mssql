...
do
    case "${flag}" in
        ...
        h) echo "Add description of the script functions here."
           ...
           echo "-u     Pass the user name by parameter."
           echo
           exit 1;;
...
docker exec -it "$CONTAINER_NAME" /opt/mssql-tools/bin/sqlcmd -b -V16 -S localhost -U $USER_NAME -Q "BACKUP DATABASE [$DATABASE_NAME] TO DISK = N'/var/opt/mssql/backups/$FILE_NAME' with NOFORMAT, NOINIT, NAME = '$DATABASE_NAME-full', SKIP, NOREWIND, NOUNLOAD, STATS = 10"
